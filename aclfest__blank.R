...
# you will need to install the tidyverse package if you've not
# done so already - see Lesson 1
library(tidyverse)
...
# this is: 0.26009693 + 0.44588045 - 0.04523425 = 0.66
      ...
      0.26009693 + 0.44588045 - 0.04523425
###
...
# Add a step in the pipeline to round to two decimal places.
      ...
      xtabs(~outsidelands + bonnaroo, data = aclfest) %>%
        prop.table %>%
        addmargins %>%
        round (3)

# Checking independence from data #
##### We will run through this on Thursday #####
...
## P (coachella | outsidelands = 1) = P (coachella)
      library (tidyverse)
      ...
      xtabs (~coachella + outsidelands, data = aclfest) %>%
        ...
        xtabs (~coachella + acl, data = aclfest) %>%
          prop.table(margin = 2) %>%
          round (3)
      ## use outsidelands so type in 2
